# ===============================
# DGM Master Builder v4.5 - COMPLETE & CORRECTED
# Wave files filtered by MailboxMigration=YES, routing only for non-elkw.local
# Added: Seafile per-wave exports, extensionattribute7=Wave, configurable max wave
# ===============================
...
# === WAVE CONFIGURATION ===
$MaxWave = 14                    # Maximum wave number (W1-W14)
...
$OutDir_Seafile      = Join-Path $RunRoot "Seafile"
New-Item -ItemType Directory -Path $RunRoot,$OutDir_Master,$OutDir_Waves,$OutDir_Seafile -Force | Out-Null
...
# Helper: Convert Wave to extensionattribute7 format
function Get-ExtensionAttribute7 {
    param([string]$Wave)
    if ($Wave -eq 'Pilot') { return 'Pilot' }
    if ($Wave -match '^W(\d+)$') { return "Wave$($matches[1])" }
    return ""
}
...
foreach ($u in $cancom) {
    try {
        ...
        # Get CT_Instanz and CT_Designation for this user's KROs
        $userKROs = @()
        if ($kro_new) { $userKROs += (CanonKRO $kro_new 3) }
        if ($kro_old) { $userKROs += ($kro_old -split ',' | ForEach-Object { CanonKRO $_ 3 } | Where-Object { $_ }) }
        $userKROs = $userKROs | Where-Object { $_ } | Sort-Object -Unique
        
        $ctInstanzUser = GetCTInstanzListByKROs $userKROs $ct_hash
        $ctDesignationUser = ""
        foreach ($k in $userKROs) {
            if ($ct_hash.ContainsKey($k)) {
                $d = $ct_hash[$k].designation
                if ($d -and -not $ctDesignationUser) { $ctDesignationUser = $d }
            }
        }
        ...
        # extensionattribute7 = Wave format (Wave1..Wave14 or Pilot)
        $extAttr7 = Get-ExtensionAttribute7 -Wave $waveInfo.Wave
        ...
        $row = [PSCustomObject]@{
            ...
            CTInstanz              = $ctInstanzUser
            CT_Designation         = $ctDesignationUser
...
# SHARED MAILBOX PROVISIONING STATUS LOGIC (after building masterSharedMailboxes)
foreach ($smb in $masterSharedMailboxes) {
    $email = $smb.funktionspostfach.ToLower().Trim()
    
    # Check Quick2 status
    $quick2Status = $migrationStatusByEmail[$email]
    ...
    if ($quick2Status -eq 'Synced') {
        $provStatus = "MIGRATED"
    } elseif ($quick2Status -eq 'Failed') {
        # Check wave to determine if Nachzügler
        $wRaw = $smb.Wave -replace 'W|Pilot',''
        if ($wRaw -match '^\d+$') {
            $waveNum = [int]$wRaw
            if ($waveNum -lt $CurrentBatchThreshold) {
                $provStatus = "NACHZUEGLER_FAILED"
            } else {
                $provStatus = "FAILED_FUTURE_WAVE"
            }
        ...
        # Not in Quick2 - check if past wave
        $wRaw = $smb.Wave -replace 'W|Pilot',''
        if ($wRaw -match '^\d+$') {
            $waveNum = [int]$wRaw
            if ($waveNum -lt $CurrentBatchThreshold) {
                $provStatus = "NACHZUEGLER_PENDING"
            } else {
                $provStatus = "OK_FUTURE_WAVE"
            }
        } elseif ($smb.Wave -eq 'Pilot') {
    ...
    $smb | Add-Member -NotePropertyName ProvisioningStatus -NotePropertyValue $provStatus -Force
    $smb | Add-Member -NotePropertyName MigrationStatus_Quick2 -NotePropertyValue $(if ($quick2Status) { $quick2Status } else { "Not in Quick2" }) -Force
}
...
# === EXPORT MASTER SEAFILE FILE (ALL SEAFILE=TRUE users) ===
Write-Host "Exporting master Seafile file..."
$seafileColumns = @('Wave','email','gkz','gkz_old','OREID','ObjectID','Gemeinde','CTInstanz','CT_Designation','extensionattribute7')
$masterSeafile = @($masterUsers | Where-Object { 
    $_.SEAFILE -eq 'TRUE' -and 
    $_.MigrationStatus -ne 'AUSGESCHIEDEN'
})
if ($masterSeafile.Count -gt 0) {
    $MasterSeafileCsv = Join-Path $OutDir_Master "Master_Seafile.csv"
    $masterSeafile | Select-Object $seafileColumns | Export-Csv -Path $MasterSeafileCsv -Delimiter $Delimiter -Encoding UTF8 -NoTypeInformation
    Strip-Quotes $MasterSeafileCsv
    Write-Host "  Exported $($masterSeafile.Count) Seafile users"
}
...
if ($nachzueglerUnprov.Count -gt 0) {
    ...
    Strip-Quotes $nFile
    Write-Host "    └─ Nachzügler (W1-W$($CurrentBatchThreshold - 1)): $($nachzueglerUnprov.Count)"
}

# Current Waves (W10-W14)
foreach ($i in $CurrentBatchThreshold..$MaxWave) {
    $w = "W$i"
...
if ($nachzuegler.Count -gt 0) {
    Write-Host "  Nachzügler Users (W1-$($CurrentBatchThreshold - 1)/Pilot, ready for batch): $($nachzuegler.Count)"
    
    # Full detail export
    $nzFile = Join-Path $OutDir_Master "Nachzuegler_W1-$($CurrentBatchThreshold - 1)_Ready.csv"
...
# === NACHZÜGLER SEAFILE (Past Wave, SEAFILE=TRUE, NOT MIGRATED) ===
$nachzueglerSeafile = @($masterUsers | Where-Object {
    $wRaw = $_.Wave -replace 'W|Pilot',''
    $isPastWave = if ($wRaw -match '^\d+$') { [int]$wRaw -lt $CurrentBatchThreshold } elseif ($_.Wave -eq 'Pilot') { $true } else { $false }
...
if ($nachzueglerSeafile.Count -gt 0) {
    Write-Host "  Nachzügler Seafile (W1-$($CurrentBatchThreshold - 1)/Pilot): $($nachzueglerSeafile.Count)"
    
    $nzsFile = Join-Path $OutDir_Seafile "Nachzuegler_Seafile_W1-$($CurrentBatchThreshold - 1).csv"
    $nachzueglerSeafile | Select-Object $seafileColumns | 
        Export-Csv -Delimiter $Delimiter -Encoding UTF8 -NoTypeInformation -Path $nzsFile
    Strip-Quotes $nzsFile
}

# === NACHZÜGLER SHARED MAILBOXES ===
$nachzueglerShared = @($masterSharedMailboxes | Where-Object {
    $wRaw = $_.Wave -replace 'W|Pilot',''
    $isPastWave = if ($wRaw -match '^\d+$') { [int]$wRaw -lt $CurrentBatchThreshold } elseif ($_.Wave -eq 'Pilot') { $true } else { $false }
...
if ($nachzueglerShared.Count -gt 0) {
    Write-Host "  Nachzügler Shared Mailboxes (W1-$($CurrentBatchThreshold - 1)/Pilot, ready for batch): $($nachzueglerShared.Count)"
    
    # Full detail export
    $nzsFile = Join-Path $OutDir_Master "Nachzuegler_SharedMailboxes_W1-$($CurrentBatchThreshold - 1)_Ready.csv"
    $nachzueglerShared | Export-Csv -Delimiter $Delimiter -Encoding UTF8 -NoTypeInformation -Path $nzsFile
    Strip-Quotes $nzsFile
    
    # Quick2 Format
    $qsFile = Join-Path $OutDir_Master "Nachzuegler_SharedMailboxes_Quick2_Batch.csv"
...
if ($routingFailuresAll.Count -gt 0) {
    ...
    if ($pastWaveRoutingIssues.Count -gt 0) {
        Write-Host "    └─ W1-W$($CurrentBatchThreshold - 1)/Pilot still routing to Kopano: $($pastWaveRoutingIssues.Count)"
    }
    if ($futureWaveRoutingIssues.Count -gt 0) {
        Write-Host "    └─ W$CurrentBatchThreshold+ NOT routing to Kopano: $($futureWaveRoutingIssues.Count)"
    }
} else {
...
$wavesOrdered = (1..$MaxWave | ForEach-Object { "W$_" }) + "Pilot"
$joinedRows = @()
$activeRows = @()
$inactiveRows = @()
...
foreach ($w in $wavesOrdered) {
    ...
    # --- Export Wave Seafile (ONLY SEAFILE=TRUE) ---
    $usersWaveSeafile = @($usersWaveAll | Where-Object { $_.SEAFILE -eq 'TRUE' })
    ...
    # === SEAFILE PER-WAVE EXPORTS ===
    if ($usersWaveSeafile.Count -gt 0) {
        $fSuffix = if ($w -eq "Pilot") { 
            "Pilot" 
        } else { 
            "{0:D2}" -f [int]$w.Substring(1) 
        ...
        $seafileFile = Join-Path $OutDir_Seafile ("Wave_{0}_Seafile.csv" -f $fSuffix)
        $usersWaveSeafile | Select-Object $seafileColumns | 
            Export-Csv -Path $seafileFile -Delimiter $Delimiter -Encoding UTF8 -NoTypeInformation
        Strip-Quotes $seafileFile
    }
...
$joinedCsv   = Join-Path $OutDir_Master "Gemeinden_W1-${MaxWave}_ALL.csv"
$activeCsv   = Join-Path $OutDir_Master "Gemeinden_W1-${MaxWave}_AKTIV.csv"
$inactiveCsv = Join-Path $OutDir_Master "Gemeinden_W1-${MaxWave}_INAKTIV.csv"
...
try {
    ...
    # Migration_Seafile.xlsx (ONLY users with SEAFILE=TRUE)
    foreach ($i in 1..$MaxWave) {
        $w = "W$i"
        $sheetName = "Welle" + "{0:D2}" -f $i
        $seafileCsv = Join-Path $OutDir_Seafile ("Wave_{0:D2}_Seafile.csv" -f $i)
        
        if (Test-Path $seafileCsv) {
            $users = Import-Csv $seafileCsv -Delimiter $Delimiter
            if ($users -and $users.Count -gt 0) {
                $users | Export-Excel -Path $MigrationSeafilePath -WorksheetName $sheetName -AutoSize -TableName $sheetName -NoNumberConversion:$true -TableStyle Medium2 -Append
            }
        }
    }
    
    # Add Pilot sheet for Seafile
    $seafileCsvPilot = Join-Path $OutDir_Seafile "Wave_Pilot_Seafile.csv"
    if (Test-Path $seafileCsvPilot) {
        $users = Import-Csv $seafileCsvPilot -Delimiter $Delimiter
        if ($users -and $users.Count -gt 0) {
            $users | Export-Excel -Path $MigrationSeafilePath -WorksheetName "Pilot" -AutoSize -TableName "Pilot" -NoNumberConversion:$true -TableStyle Medium2 -Append
        }
    }
    
    # Add Nachzügler Seafile sheet
    $nachzueglerSeafileCsv = Join-Path $OutDir_Seafile "Nachzuegler_Seafile_W1-$($CurrentBatchThreshold - 1).csv"
    if (Test-Path $nachzueglerSeafileCsv) {
        $users = Import-Csv $nachzueglerSeafileCsv -Delimiter $Delimiter
        if ($users -and $users.Count -gt 0) {
            $users | Export-Excel -Path $MigrationSeafilePath -WorksheetName "Nachzuegler" -AutoSize -TableName "Nachzuegler" -NoNumberConversion:$true -TableStyle Medium2 -Append
        }
    }
...
# 15. --- SUMMARY ---
Write-Host "`n========================================="
Write-Host "✅ DGM Master Builder v4.5 - COMPLETE"
Write-Host "========================================="
Write-Host "`nConfiguration:"
Write-Host "  MaxWave = $MaxWave (W1-W$MaxWave)"
Write-Host "  CurrentBatchThreshold = W$CurrentBatchThreshold"
...
Write-Host "    - Master_Seafile.csv" -NoNewline
if ($masterSeafile.Count -gt 0) { Write-Host " [$($masterSeafile.Count) users]" } else { Write-Host " (empty)" }
...
Write-Host "    - Unprovisioned_Users_AD_Create_Nachzuegler.csv (W1-W$($CurrentBatchThreshold - 1))" -NoNewline
if ($nachzueglerUnprov.Count -gt 0) { Write-Host " [$($nachzueglerUnprov.Count) users]" } else { Write-Host " (none)" }
for ($i = $CurrentBatchThreshold; $i -le $MaxWave; $i++) {
...
Write-Host "    - Nachzuegler_W1-$($CurrentBatchThreshold - 1)_Ready.csv" -NoNewline
if ($nachzuegler.Count -gt 0) { 
...
if ($nachzueglerShared.Count -gt 0) {
    Write-Host "    - Nachzuegler_SharedMailboxes_W1-$($CurrentBatchThreshold - 1)_Ready.csv [$($nachzueglerShared.Count) shared ready]"
...
Write-Host "    - Gemeinden_W1-${MaxWave}_ALL.csv"
Write-Host "    - Gemeinden_W1-${MaxWave}_AKTIV.csv"
Write-Host "    - Gemeinden_W1-${MaxWave}_INAKTIV.csv"
Write-Host "`n📦 Per Wave Files (W01-W$MaxWave + Pilot):"
...
Write-Host "`n📂 Seafile Directory:"
Write-Host "  - Nachzuegler_Seafile_W1-$($CurrentBatchThreshold - 1).csv" -NoNewline
if ($nachzueglerSeafile.Count -gt 0) { Write-Host " [$($nachzueglerSeafile.Count) users]" } else { Write-Host " (none)" }
Write-Host "  Per Wave Seafile (W$CurrentBatchThreshold-W$MaxWave + Pilot):"
for ($i = $CurrentBatchThreshold; $i -le $MaxWave; $i++) {
    $sf = Join-Path $OutDir_Seafile ("Wave_{0:D2}_Seafile.csv" -f $i)
    if (Test-Path $sf) {
        $cnt = (Import-Csv $sf -Delimiter $Delimiter).Count
        Write-Host "    - Wave_$("{0:D2}" -f $i)_Seafile.csv [$cnt users]"
    }
}
$sfPilot = Join-Path $OutDir_Seafile "Wave_Pilot_Seafile.csv"
if (Test-Path $sfPilot) {
    $cnt = (Import-Csv $sfPilot -Delimiter $Delimiter).Count
    Write-Host "    - Wave_Pilot_Seafile.csv [$cnt users]"
}
Write-Host "`n📊 Excel Workbooks (if ImportExcel available):"
Write-Host "  - Migration_Gemeinden.xlsx"
Write-Host "  - Migration_Seafile.xlsx (with Nachzügler + per-wave sheets)"
...
Write-Host "`nSeafile Status:"
$seafileTotalCount = ($masterUsers | Where-Object { $_.SEAFILE -eq 'TRUE' -and $_.MigrationStatus -ne 'AUSGESCHIEDEN' }).Count
Write-Host "  - Total Seafile users: $seafileTotalCount"
Write-Host "  - Nachzügler Seafile (W1-W$($CurrentBatchThreshold - 1)): $($nachzueglerSeafile.Count)"
...
Write-Host "`nSeafile Wave Distribution (SEAFILE=TRUE):"
Write-Host "  - Pilot Seafile: $(($masterUsers | Where-Object { $_.Wave -eq 'Pilot' -and $_.SEAFILE -eq 'TRUE' -and $_.MigrationStatus -ne 'AUSGESCHIEDEN' }).Count) users"
for ($i = 1; $i -le $MaxWave; $i++) {
    $cnt = ($masterUsers | Where-Object { $_.Wave -eq "W$i" -and $_.SEAFILE -eq 'TRUE' -and $_.MigrationStatus -ne 'AUSGESCHIEDEN' }).Count
    if ($cnt -gt 0) {
        $marker = if ($i -lt $CurrentBatchThreshold) { "N" } elseif ($i -eq $CurrentBatchThreshold) { "→" } else { " " }
        Write-Host "  $marker W$($i.ToString('D2')): $cnt users"
    }
}
Write-Host "`nShared Mailboxes: $($masterSharedMailboxes.Count) (with full KRO/Gemeinde data)"
if ($masterSharedMailboxes.Count -gt 0) {
    $smbMigrated = ($masterSharedMailboxes | Where-Object { $_.ProvisioningStatus -eq 'MIGRATED' }).Count
    $smbNachzuegler = ($masterSharedMailboxes | Where-Object { $_.ProvisioningStatus -match 'NACHZUEGLER' }).Count
    $smbPending = ($masterSharedMailboxes | Where-Object { $_.ProvisioningStatus -match 'PENDING|OK_FUTURE' }).Count
    Write-Host "  ✓ MIGRATED: $smbMigrated"
    Write-Host "  ⚠ NACHZUEGLER: $smbNachzuegler"
    Write-Host "  - PENDING: $smbPending"
}
Write-Host "========================================="
